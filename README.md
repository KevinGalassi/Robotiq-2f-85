...
All the information of the gripper can be obtained [here](https://robotiq.com/support/2f85-140-adaptive-robot-gripper) from the website of the robotiq
...
For use in `Kinetic` and `Melodic` consider to change the shebang to set the correct environment path to python instead python3.
...
To use the gripper you can create an instance of `Robotiq2FingerGripperDriver` to create a publisher to the action server of the gripper and use the set of function to control the gripper:

* move : Move the gripper to the target position
* close : Completly close the gripper
* open : Full open the gripper

To launch the action server of the gripper use :

---

```
# Connect and move 85 mm gripper  
roslaunch robotiq_2f_gripper_control robotiq_action_server.launch  comport:="<YOUR GRIPPER TTY PORT NAME>"
# Default comport is /dev/ttyUSB0
roslaunch robotiq_2f_gripper_control robotiq_action_server.launch comport:="/dev/ttyUSB0"
```

The status led will switch from 'red' to 'blue', meaning that the gripper is ready to take command.
